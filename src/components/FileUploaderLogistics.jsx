...
// Lógica de Processamento (Adaptada do seu exemplo)
const processSheet = (sheetData, sheetName, userId, sheetIndex, fileType) => {
    console.log(`[processSheet - FileUploaderLogistics] Iniciando aba "${sheetName}" (Índice ${sheetIndex}) - Tipo: ${fileType}`);
    const metrics = [];
    ...
    for (let r = 0; r < Math.min(sheetData.length, 15); r++) {
        ...
        if (isLikelyHeaderRow) {
            ...
            if (validDatesFound >= MIN_VALID_DATES_IN_HEADER) { dateColumnsMap = potentialDateMap; headerRowIndex = r; console.log(`%c[Sheet ${sheetIndex}] Linha Cabeçalho Data índice ${r}. Datas Mapeadas: OK`, 'color: blue; font-weight: bold;'); break; }
        }
    }
    if (headerRowIndex === -1) { console.error(`[Sheet ${sheetIndex}] Nenhuma linha cabeçalho com datas válidas.`); return []; }
    ...
    for (let rowIndex = headerRowIndex + 1; rowIndex < sheetData.length; rowIndex++) {
        ...
        if (sheetIndex === 0) {
            ...
            else if (rowLabelUpper === 'GERAL' || rowLabelUpper === 'TOTAL') { if (currentCategory === 'ENTREGA / REGIÃO') { currentCategory = null; } isSectionHeader = true; } // Mantém outros contextos
        } else { // sheetIndex === 1
        ...
        if ((sheetIndex === 0 && currentCategory) || (sheetIndex === 1 && currentRegion)) {
            if (Object.keys(dateColumnsMap).length > 0) {
                ...
                } else { // Aba Estado/Região
                    const state = rowLabel.substring(0, 50); // Limita o estado
                    const stateUpper = state.toUpperCase(); // Usa valor limitado para verificar
                    if (regionStateMap[currentRegion]?.map(s=>s.toUpperCase()).includes(stateUpper)) { // Verifica se estado pertence à região
                         const metricKey = 'processed_accumulated'; // Chave para dados consolidados por estado
                        for (const colIndexStr in dateColumnsMap) { const colIndex = parseInt(colIndexStr, 10); const metricDateStr = dateColumnsMap[colIndex]; if (metricDateStr && colIndex < row.length) { const value = parseBrazilianNumberInternal(row[colIndex]); if (value !== null) { const uniqueKey = `${metricDateStr}-${stateUpper}-${metricKey}`; if (!uniqueKeys.has(uniqueKey)) { metrics.push({ metric_date: metricDateStr, region: currentRegion, state: state, metric_key: metricKey, value: value, source_file_type: 'logistics_state_consolidated', uploaded_by: userId }); uniqueKeys.add(uniqueKey); } } } }
        ...
        } else if (!isSectionHeader) { console.warn(`[Sheet ${sheetIndex}] Linha ${rowIndex + 1} ("${rowLabel}"): Ignorada - Contexto Categoria/Região não está definido.`); }
    }
    console.log(`[processSheet - FileUploaderLogistics] Finalizado aba "${sheetName}". Métricas: ${metrics.length}`);
    return metrics;
};

// Função de Leitura Adaptada
const readAndProcessLogisticsExcel = async (fileToRead, userId, fileType) => { console.log(`[readAndProcess - FileUploaderLogistics] Lendo: ${fileToRead.name} (Tipo: ${fileType})`); const reader = new FileReader(); const fileData = await new Promise((resolve, reject) => { reader.onload = (e) => resolve(e.target.result); reader.onerror = reject; reader.readAsArrayBuffer(fileToRead); }); const workbook = XLSX.read(fileData, { type: 'array', cellDates: true, cellNF: true, cellStyles: false }); let consolidatedMetrics = [], stateMetrics = []; if (workbook.SheetNames.length > 0) { const sheetName0 = workbook.SheetNames[0]; const worksheet0 = workbook.Sheets[sheetName0]; if (worksheet0) { const sheetData0 = XLSX.utils.sheet_to_json(worksheet0, { header: 1, raw: false, defval: null }); console.log(`[readAndProcess] Aba 0: "${sheetName0}" (${sheetData0.length} linhas)`); consolidatedMetrics = processSheet(sheetData0, sheetName0, userId, 0, fileType); } } if (workbook.SheetNames.length > 1) { const sheetName1 = workbook.SheetNames[1]; const worksheet1 = workbook.Sheets[sheetName1]; if (worksheet1) { const sheetData1 = XLSX.utils.sheet_to_json(worksheet1, { header: 1, raw: false, defval: null }); console.log(`[readAndProcess] Aba 1: "${sheetName1}" (${sheetData1.length} linhas)`); stateMetrics = processSheet(sheetData1, sheetName1, userId, 1, fileType); } } return { consolidatedMetrics, stateMetrics }; }; // Renomeado para stateMetrics
...
function FileUploaderLogistics({ onFileUpload, user, onClose }) {
    ...
    const handleUpload = async () => {
        ...
        try {
            // Chama readAndProcess com fileType 'consolidated'
            const processedData = await readAndProcessLogisticsExcel(file, user.id, 'consolidated');
            console.log("[FileUploaderLogisticsConsolidated] Dados processados:", { consolidatedCount: processedData.consolidatedMetrics?.length ?? 0, stateCount: processedData.stateMetrics?.length ?? 0 });

            if ((!processedData.consolidatedMetrics || processedData.consolidatedMetrics.length === 0) && (!processedData.stateMetrics || processedData.stateMetrics.length === 0)) {
                throw new Error("Nenhum dado válido (consolidado ou estado/região) foi extraído.");
            }

            console.log("[FileUploaderLogisticsConsolidated] Enviando dados CONSOLIDADOS (e estado acumulado) para Supabase...");
            const results = await Promise.allSettled([
                upsertLogisticsConsolidatedMetrics(processedData.consolidatedMetrics), // Salva na tabela CONSOLIDADA
                insertLogisticsDailyMetrics(processedData.stateMetrics)           // Salva na tabela de ESTADO (com metric_key='processed_accumulated')
            ]);

            const consolidatedResult = results[0];
            const stateResult = results[1];
            let errors = [];
            let successSummary = { processedConsolidated: 0, processedState: 0 };

            if (consolidatedResult.status === 'rejected') { errors.push(`Erro Consolidado: ${consolidatedResult.reason?.message || '?'}`); }
            else if (consolidatedResult.value?.error) { errors.push(`Erro Consolidado: ${consolidatedResult.value.error.message || '?'}`); }
            else { successSummary.processedConsolidated = consolidatedResult.value?.data?.length ?? 0; }

            if (stateResult.status === 'rejected') { errors.push(`Erro Estado/Região: ${stateResult.reason?.message || '?'}`); }
            else if (stateResult.value?.error) { errors.push(`Erro Estado/Região: ${stateResult.value.error.message || '?'}`); }
    ...
    return (
         <div className="file-upload-container border-2 border-dashed border-gray-300 p-6 rounded-lg bg-gray-50 text-center relative" data-name="logistics-consolidated-file-uploader">
             ...
             <div className="file-upload-text mb-4">
                 ...
                 <p className="text-xs text-gray-500 mt-1">(Processará 1ª aba como Consolidado Acum. e 2ª como Estado/Região Acum.)</p>
             </div>
